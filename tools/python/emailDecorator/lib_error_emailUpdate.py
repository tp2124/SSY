...
LIST_OF_RECIPIENTS = [Contact('Travis Primm', 'tp2124@gmail.com', '2145324336', 'AT&T'),
                        Contact('Conrad Kurth', 'conradkurth@gmail.com', '8056980048', ''), #his is Verizon
                        ]
...
def emailError(message, sourceTraceback):
    ...
    for boss in LIST_OF_RECIPIENTS:
        text = 'From: [Scrapyard Tool Error] <scrapyardtools@gmail.com>\nTo:Tool Master <' +\
                boss.emailAddr + '>\nSubject: Problem with: %s\n\n' % scriptPath + message
        server.sendmail('scrapyardtools@gmail.com', boss.emailAddr, text)
        if boss.phoneProvider:
            #Send text
            textEmailAddr = boss.phoneNumber
            if boss.phoneProvider == 'AT&T':
                textEmailAddr += '@txt.att.net'
            elif boss.phoneProvider == 'Verizon':
                textEmailAddr += '@vtext.com'
            elif boss.phoneProvider == 'T-Mobile':
                textEmailAddr += '@tmomail.net'
            elif boss.phoneProvider == 'Sprint':
...
def emailNotifyer(original_function):
    ...
    # If youre lazy, think of wrapping other code in this
    # And this catches and exceptions and emails back with error details
    # Only used when seeing the @emailNotifer above other functions
    def wrapper(*args, **kwargs):
        try:
            ...
            #emailMsg = '\n\n___Traceback___\n\tWhat file the Error Came From: ' + str(originalError.func_code.co_filename)
