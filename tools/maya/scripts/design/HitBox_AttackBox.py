...
class AnimationFrame(object):
    ...
    def fromJson(self,data, useRootOffsetData=False, useZ=True, useY=True):
        ...
        #Do the same for attackboxes sir
...
class CollisionBox(object):
    ...
    def fromJson(self, data, useRootOffsetData=False, useZ=True, useY=True):
        ...
        #cmds.polyCube( self.m_name, e=True, w = self.m_width, h = self.m_height)
...
def saveJson(absFilePath,data):
    ...
    #TODO1: Make this use the forceWriteable function in utils.sm_force
    #TOOD2: Make this perforce proper once Maya integration is good.
...
def validateFrame():
    ...
    if g_currentFrame != frame:
        #removeBoxesFromView(g_currentFrame)
...
def removeHitBox(self):
    ...
    #for index in len(g_animationFrames[frame].m_hitBoxes):
    #    cmds.delete(g_animationFrames[frame].m_hitBoxes[index].m_name)
    #g_animationFrames[frame].m_hitBoxes = [] #not sure if this is good practice
...
def removeAttackBox(self):
    validateFrame()
    #for index in len(g_animationFrames[frame].m_attackBoxes):
    #    cmds.delete(g_animationFrames[frame].m_attackBoxes[index].m_name)
    #g_animationFrames[frame].m_attackBoxes = [] #not sure if this is good practice
...
def removeBoxesFromView(currentFrame):
    # print "Trying to remove all the boxes from the view"
    for index in range(len(g_animationFrames[currentFrame].m_hitBoxes)):
        if g_animationFrames[currentFrame].m_hitBoxes[index] != None:
...
def showNewBoxesInView(frame):
    # print "Trying to show the corresponding boxes for frame: %d" % frame
